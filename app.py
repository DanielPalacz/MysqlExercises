...
from src.helpers import (
    get_credentials,
    build_update_currency_transaction,
    transaction_reader,
    get_file_content,
)
...
def db_setup(config) -> None:
    ...
    with DbWrapper(**creds) as db:
        click.secho(
            message=" ... Initializing fresh mydb instance.", fg="blue", bold=False
        )
        db.initiate_db()
    ...
    with DbWrapper(**creds) as db:
        click.secho(
            message=" ... Adding 2 new columns to Product table (UnitPriceUSD, UnitPriceEuro).",
            fg="blue",
            bold=False,
        )
...
def generate_report(config):
    ...
    click.secho(
        message=f" ... Product report {report_name!r} was generated.", fg="blue"
    )
...
def update_currency_rates(config) -> None:
    ...
    click.secho(
        message=" ... Downloading current currency rate for USD.", fg="blue", bold=False
    )
    usd_rate = nbp_client.get_mid_currency_rate("usd")
    click.secho(
        message=" ... Downloading current currency rate for Euro.",
        fg="blue",
        bold=False,
    )
    ...
    with DbWrapper(**get_credentials()) as db:
        click.secho(
            message=" ... Updating mydb.Product.UnitPriceUSD column with current USD rate.",
            fg="blue",
            bold=False,
        )
        db.execute_query(
            build_update_currency_transaction(currency="USD", value=usd_rate)
        )
    with DbWrapper(**get_credentials()) as db:
        click.secho(
            message=" ... Updating mydb.Product.UnitPriceEuro column with current Euro rate.",
            fg="blue",
            bold=False,
        )
        db.execute_query(
            build_update_currency_transaction(currency="Euro", value=euro_rate)
        )
